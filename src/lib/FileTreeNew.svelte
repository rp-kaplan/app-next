<script lang="ts">
  import { readDir, writeTextFile, createDir } from "@tauri-apps/plugin-fs";
  ...
  function renderTreeItem(item: FileItem, level: number = 0) {
    return {
      item,
      level,
      getFileIcon,
...
{#snippet TreeItem({ item, selectedFile, getFileIcon, level = 0, ...props })}
